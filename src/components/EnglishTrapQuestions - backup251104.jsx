...
export default function EnglishTrapQuestions() {
  ...
  const renderUnitButton = (unit) => {
    ...
    // 背景カラー
    let bgClass = "bg-white border border-gray-300 hover:bg-gray-100";
    if (mode === 1)
      bgClass =
        "bg-gradient-to-b from-green-300 to-green-500 border-green-500 shadow-md hover:scale-[1.03]";
    else if (mode === 2)
      bgClass =
        "bg-gradient-to-b from-blue-300 to-blue-500 border-blue-500 shadow-md hover:scale-[1.03]";
    else if (mode === 3)
      bgClass =
        "bg-gradient-to-b from-orange-300 to-orange-500 border-orange-500 shadow-md hover:scale-[1.03]";

    // 文字色（モード別）
    const textColor = mode === 0 ? "text-gray-800" : "text-white";
      ...
      >
        ...
        <span
          ...
          style={{
            ...
            fontSize:
              unit.length >= 8 ? "10px" : unit.length >= 6 ? "12px" : "14px",
          }}
        >
          {unit}
        </span>
    ...
    <div className="min-h-screen bg-gradient-to-r from-pink-100 to-yellow-100 max-w-4xl mx-auto p-4">
      ...
      {!showQuestions && !showResult && units.length > 0 && (
        <div className="relative min-h-screen flex flex-col justify-start items-center overflow-hidden bg-gradient-to-br from-[#E0F7FA] via-white to-[#F1F8E9]">
          ...
          {/* 🏷️ タイトル＋質問ボックス */}
          <header className="relative flex flex-col sm:flex-row items-center justify-center text-center mt-6 mb-8 z-10 gap-2 sm:gap-4 w-full px-4">
          ...
          {/* 🧩 メインカード（全幅化＋背景透過） */}
          <main className="w-full bg-white/80 backdrop-blur-lg shadow-inner p-4 sm:p-6 z-10 rounded-none border-t border-b border-[#E0E0E0]">
            {/* === 出題形式タブ === */}
                    ...
                    >
                      {Array.from(
                        ...
                        const displayName = unit
                          .replace("単語テスト", "")
                          .trim();
                        return renderUnitButton(displayName || unit);
                      })}
                    </motion.div>
                  )}
                </AnimatePresence>
